...
TPersonasABB deserializarTPersonasABB(TPilaPersona &pilaPersonas)
{
  ...
  TColaPersonasABB queue = crearTColaPersonasABB();
  insertarTPersonasABB(newTree, copiarTPersona(cimaDeTPilaPersona(pilaPersonas)));
  encolarEnTColaPersonasABB(newTree, queue);
  desapilarDeTPilaPersona(pilaPersonas);

  while (cantidadEnTPilaPersona(pilaPersonas) > 0)
  {
    for (nat i = 0; i < cantidadEnTColaPersonasABB(queue); i++)
    {
      TPersonasABB current = frenteDeTColaPersonasABB(queue);
      if (cantidadEnTPilaPersona(pilaPersonas) > 0)
      {
        TPersonasABB left = crearTPersonasABB();
        insertarTPersonasABB(left, copiarTPersona(cimaDeTPilaPersona(pilaPersonas)));
        current->left = left;
        encolarEnTColaPersonasABB(left, queue);
        desapilarDeTPilaPersona(pilaPersonas);
      }
      if (cantidadEnTPilaPersona(pilaPersonas) > 0)
      {
        TPersonasABB right = crearTPersonasABB();
        insertarTPersonasABB(right, copiarTPersona(cimaDeTPilaPersona(pilaPersonas)));
        current->right = right;
        encolarEnTColaPersonasABB(right, queue);
        desapilarDeTPilaPersona(pilaPersonas);
      }
      desencolarDeTColaPersonasABB(queue);
    }
